...
public sealed class AddThreeBlock(IRunner runner) : PipeBlockBase<int, int, AddThreeOption>
{
    ...
    public override async Task<ComputeResult<int>> ProcessAsync(int input, AddThreeOption option, CancellationToken cancellationToken = default)
    {
        ...
        var sortie = await runner.RunAsync(pipe.GetDetails(), input, cancellationToken);
    ...
    public override async Task<IList<ProcessStep>> ExplainAsync(Etape context, CancellationToken ct)
    {
        ...
        var sortie = await runner.ExplainAsync(pipe.GetDetails(), ct);
